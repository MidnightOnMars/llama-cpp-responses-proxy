...
An SSE compatibility proxy for llama.cpp's `/v1/responses` streaming endpoint. It patches a small set of missing OpenAI Responses fields so strict clients such as `@ai-sdk/openai` stop dropping `function_call` chunks.
...
If your client can use `@ai-sdk/openai-compatible` or otherwise target `/v1/chat/completions`, that is the cleaner option today.

llama.cpp's Chat Completions streaming already works for tool calling, so no proxy is needed:
...
The proxy is only for clients that force the Responses path. For `@ai-sdk/openai`, that still happens unconditionally via `createResponsesModel()`.
...
Non-SSE requests pass through unchanged. For SSE streams, the proxy parses each event block, injects the missing fields, and forwards the patched event.

The current implementation also:

- supports both `http://` and `https://` upstream targets
- handles LF and CRLF SSE framing
- includes a small `node:test` suite for the patching logic
